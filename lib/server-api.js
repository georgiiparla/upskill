...
const processResponse = async (response, path) => {
    if (!response.ok) {
        console.log(`[ServerFetch] Request to ${path} failed with status ${response.status}`);
        if (response.status === 401) {
            // server-side redirect to login for unauthorized
            // TEMPORARILY DISABLED FOR DEBUGGING
            // redirect('/login');
            console.log('[ServerFetch] 401 detected. Token was present:', (await getTokenFromCookies()) ? 'YES' : 'NO');
        }
