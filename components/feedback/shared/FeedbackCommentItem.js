...
export const FeedbackCommentItem = ({ feedback }) => {
    ...
    const sentimentColors = {
        Positive: "border-green-500 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400",
        Negative: "border-red-500 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400",
        Neutral: "border-amber-500 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400",
    };
    const colorClass = sentimentColors[feedback.sentiment] || sentimentColors.Neutral;

    return (
        <Card className={`transition-shadow hover:shadow-md ${colorClass}`}>
            <div className="flex flex-col space-y-3">
                {/* Header: Author and Sentiment */}
